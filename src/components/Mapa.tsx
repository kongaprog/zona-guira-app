...
export const Mapa = ({ negocios, alSeleccionar }: Props) => {
  ...
  const crearIconoEmoji = (emoji: string) => {
    return L.divIcon({
      ...
      html: `
        <div style="background-color: #0f172a; color: white; border-radius: 50%; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; font-size: 18px; box-shadow: 0 4px 10px rgba(0,0,0,0.5); border: 2px solid #38bdf8;">
          ${emoji}
        </div>
      `,
      iconSize: [36, 36],
        ...
        {negocios.map((negocio) => {
          ...
          return (
            <Marker key={negocio.id} position={[lat, lng]} icon={crearIconoEmoji(obtenerEmoji(negocio))}>
              <Popup>
                {/* FICHA CON TAMAÑO FORZADO PARA MÓVIL */}
                <div className="min-w-[240px] max-w-[260px] bg-slate-900/95 backdrop-blur-md text-white rounded-2xl overflow-hidden shadow-2xl border border-slate-700 font-sans block">
                  
                  {/* FOTO: h-32 y block para asegurar altura */}
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-slate-400 text-xs flex items-center gap-1 mb-3 truncate">📍 {negocio.ubicacion.split(',')[0]}</p>
                    <button onClick={() => alSeleccionar(negocio)} className="w-full bg-white text-slate-900 py-2.5 rounded-lg font-bold text-xs active:bg-slate-200 transition-colors shadow-lg flex items-center justify-center gap-1">
                      Ver Perfil
                    </button>
                  </div>
                </div>
