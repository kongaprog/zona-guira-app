    ...
    <div className="pb-24 min-h-screen bg-slate-50 font-sans">
        ...
        <div className="max-w-xl mx-auto px-4 py-3">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {BOTONES_MURO.map((cat) => (
              <button key={cat} onClick={() => setFiltroActivo(cat)} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all border ${filtroActivo === cat ? "bg-slate-800 text-white border-slate-800" : "bg-white text-slate-500 border-slate-200 hover:border-slate-300"}`}>
                {cat}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-xl mx-auto p-4 flex flex-col gap-4">
        {loading ? (
          ...
          <div className="text-center py-20 opacity-50"><p>No hay anuncios de "{filtroActivo}"</p></div>
        ) : (
              ...
              <div key={anuncio.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-4 pb-2 flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-700 font-bold text-xs border border-slate-200">{inicial}</div>
                    <div>
                      <h3 className="font-bold text-slate-900 text-sm leading-none">{anuncio.nombre}</h3>
                      <div className="flex items-center gap-1 text-[10px] text-slate-400 mt-1"><Clock size={10} /> {anuncio.fecha}</div>
                    </div>
                  </div>
                  <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase border ${estilo.bg}`}>{estilo.label}</span>
                </div>
                <div className="px-4 py-2"><p className="text-slate-700 text-sm leading-relaxed whitespace-pre-wrap">{anuncio.texto}</p></div>
                {anuncio.foto && (
                  <div className="px-4 pb-2">
                    <img src={anuncio.foto} className="w-full h-auto max-h-80 object-cover rounded-lg border border-slate-100" loading="lazy" referrerPolicy="no-referrer" onError={(e) => {e.currentTarget.style.display = 'none'}} />
                  </div>
                )}
                <div className="px-4 pb-4 pt-2">
                  <a href={`https://wa.me/53${anuncio.contacto}?text=Hola, vi tu anuncio en el Muro: ${anuncio.texto.substring(0, 15)}...`} target="_blank" className="w-full bg-slate-50 hover:bg-green-50 text-slate-600 hover:text-green-700 border border-slate-200 hover:border-green-200 py-2.5 rounded-lg font-bold text-xs flex items-center justify-center gap-2 transition-all">
                    <MessageCircle size={16} className="text-green-600" /> Contactar por WhatsApp
                  </a>
                </div>
              </div>
            );
